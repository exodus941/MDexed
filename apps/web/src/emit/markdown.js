...
function componentsBody(state, derived) {
  ...
  const alignment = [
    ...
    bullets([
      ...
      '**ONE BUTTON SIZE FIXES ALL THREE: the box, the label and the icon.** Pick the triple, and every alignment question answers itself. The icon takes the type size of the label minus one step on the type scale, so it fills the label’s cap band without overflowing it: a 12px label takes a 10px icon, a 14px label a 12px, a 16px label a 14px. Two button sizes must never share a label size — if they do, they must share an icon size too, and then the larger button has a small button’s lockup sitting in more padding.',
      '**The icon lives between the CAP LINE and the BASELINE of the label beside it.** Those two lines are the whole vertical rule: the top of a capital letter and the line the letters sit on. Centre the icon’s BOX between them. Equal overhang above and below is correct — it is what centred means — and a large equal overhang is a size fault rather than an alignment one, fixed by taking the icon token down a step and never by moving the glyph.',
      '**Do not correct individual glyphs.** An icon set spans its glyphs deliberately: measured in one 12px box, a plus paints 8px of ink, a magnifier 10px and a chevron 4px. That variation is the set’s optical balance, and scaling each glyph to a common ink height destroys it while producing a table of per-glyph numbers nobody can maintain. Fix the size token and the alignment rule; leave the drawings alone.',
