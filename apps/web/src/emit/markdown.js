...
function componentsBody(state, derived) {
  ...
  const alignment = [
    ...
    bullets([
      ...
      'A fractional line box does not centre. A ratio like `1.56` on a 12.8px font computes to 19.968px, the half-leading either side becomes 0.984, and the subpixel split lands unevenly — measured at 6px above the cap against 7.95px below the baseline on a badge. Round the line box to a whole even number, with `round(1.56em, 2px)` or a whole-pixel value, and the ink centres exactly.',
      ...
      'A flex container takes its baseline from its **first flex item**, so whatever sits first inside it decides where the row\'s text sits. An `inline-flex` badge starting with a tick hands its baseline to the tick, and the same badge starting with a status dot hands it to the dot — three badges of one class, one height and one font measured 4px apart on a single line. Build a badge as an `inline-block` with a stated `line-height` and space its ornaments with `vertical-align: middle` and a margin. Then no child can move its baseline.',
      ...
      'Symmetric padding does not optically centre text, because a line box is not symmetric about the cap-to-baseline band. A taller sibling on the same baseline — a chip, a count, a badge — hangs further below that baseline than letters do, so the row grows downward only and the label it grew around ends up high inside it. Correct with unequal padding, one pixel from the bottom to the top, and only where that taller sibling is actually present. Derive the pixel by measuring, not by picking.',
      ...
      'A button holding nothing but an icon **is** icon-only, whether or not that class was typed on it. Write the structural selector beside the class one — `:has(> .icon:only-child)` — so a hand-styled copy cannot keep the leading-icon margin meant to separate a mark from a label that is not there. One that did sat 4px left of centre, exactly half the icon gap.',
      ...
      'A mark that belongs to a line of text goes **inside that line**, never in a flex slot beside it. A wrapper holding only an icon has no text to give the row a baseline, so the row invents one and every value of `align-self` lands the mark somewhere other than the line it describes — measured 3.5px high pinned to the top and 2.5px high on baseline, with nothing in that slot able to do better. Set it inline with `vertical-align: middle` and the optical lift. It then stays on the first line when the message wraps, which a flex slot never does.',
      'Anything with no text to offer — a checkbox, a switch, a progress bar, a swatch — has nothing to put on a baseline, so a baseline row aligns it by its bottom edge and it rides high beside its label.',
      'A control beside a label centres on the label\'s **first line**, never on the label as a block. Centring the row is right for one line and wrong the moment the label wraps — measured 22px out on a three-line label, because the control then centres on the paragraph instead of on the choice it belongs to. Start both at the top and push the control down by half the difference between the label\'s line height and the control\'s own height. Compute that from the label\'s type tokens rather than from the `lh` unit: `lh` resolves against whatever the control inherits, which is a different font, and it leaves a constant offset.',
      'Centring the control alone is worse than centring nothing. The box moves to the middle, the label keeps a baseline it now shares with no one, and above the text height the label climbs to the top of the row. Alignment in a pair belongs to the pair.',
      'Two spacing gaps that are stated equal do not read equal when text sits on either side. A text box carries leading, and there is more of it below a baseline than above a cap — measured 5.95 against 4.00 on a 12.8px font, so one 8px gap rendered as 13.95 and the other as 12.00. Correct the difference, or trim both boxes to their ink with `text-box: cap alphabetic`. If you trim, re-tune the spacing scale at the same time: the old values were chosen with the leading in them, and trimming alone tightens every stack at once.',
      'Proximity is decided by the RATIO, never by the absolute value. A field group whose label sits 8px from its input needs clearly more than 12px before the next group, or the help line under one field ends up closer to the next field\'s label than to its own input, and the two groups read as one. Separation must beat cohesion, and a single step on the spacing scale is often not enough to do it.',
      ...
      'An optical correction belongs to the mechanism it corrects. The small upward lift that fixes `vertical-align: middle` — about 0.12em, because middle aligns to the x-height and the eye reads the cap centre — is an ERROR of the same size when carried onto an icon already centred by flexbox. Measured 1.81px off centre with it, 0.00 without.',
