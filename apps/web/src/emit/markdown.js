...
function componentsBody(state, derived) {
  ...
  const alignment = [
    ...
    bullets([
      ...
      'Custom properties INHERIT, so a value published for a container reaches every descendant — including ones that do not own what it describes. A gap named on a page-level stack reached every card inside it, and a card that was not a stack subtracted a 16px gap it never had: the action row landed at 8px, half where it started. Reset such a property wherever the element is not the thing it describes.',
