...
function chartFurniture (derived) {
  ...
  return joinBlocks(
    ...
    '**AND A LAYER THAT CARRIES A LINE NEEDS AREA TO PAINT IT.** Put the axis and the gridlines on one absolutely positioned box and it takes its CONTENT, which for a line layer is nothing. Measured on two bar charts: 1px by 0 inside a 104px row group, with a computed inset of 52px top and 52px bottom, so the box sat collapsed at the centre of its own grid area. The axis was absent and the gradient had no box to paint in. State `inset: 0` on BOTH axes. The block side gives an axis its length, and the inline side gives a gradient its width. Every geometric check passes on this, because the box exists and sits where the grid put it.',

    '**A TICK LABEL IS CENTRED ON THE GRIDLINE IT NAMES, AND `space-between` DOES NOT DO THAT.** It distributes the label BOXES between the plot\'s edges, so the first label\'s centre sits half a line below the top gridline and the last one\'s half a line above the bottom. The middle one is right by accident. Measured on a 19.44px line: 9.72, 4.86, 0.00, -4.86, -9.72. Extend the tick column by half a line at EACH end with a negative block margin, `calc(<caption size> * <caption leading> / -2)`, and the centres land on the boundaries. That is exact rather than tuned, and it follows the type scale.',

    '**AND THAT OVERHANG LEAVES THE BOX, SO MEASURE THE INK RATHER THAN THE EDGE.** A negative margin puts the first label outside the chart, where it eats whatever clearance sits above. A card giving the title group 12px then showed 2.28. Give the chart a `padding-block` of the same half line, and only where a tick column exists: a chart without one has no overhang to absorb, and padding it is space nobody asked for.',
...
function datavizBody(derived) {
  ...
  return joinBlocks(
    ...
    /* COUNT THE COLOURS, NEVER TYPE THE NUMBER. This sentence said "eight" for
       as long as the scale had eight, and it kept saying it after the scale
       dropped to five. Every other sentence in this section already reads the
       array. */
    `**And the limit, stated.** With the red-green axis removed, that worst pair falls to **${dv.worstWithoutRedGreen.toFixed(3)}**, which is about ${(dv.worstWithoutRedGreen / 0.02).toFixed(1)} just-noticeable differences. No categorical palette of ${dv.categorical.length} is safe without red-green vision, this one included. So a chart never encodes a series by colour alone: label each series directly where it sits, or give it a dash pattern or a marker shape as well. The palette makes the picture readable and the label makes it certain.`,
    `**${dv.categorical.length} is the whole scale, and it is a limit worth keeping.** A palette that reads as one family sweeps its hue one way and arcs its lightness once, and stops on that arc get closer together the more of them there are. Measured across 36 accent hues, a sweep of ${dv.categorical.length} clears the floor everywhere and a sweep of eight clears it nowhere. A chart needing more series than this is almost always the wrong form: group the tail into an "Other", split it into small multiples, or use a shape a reader can actually follow.`,
...
function layoutBody(state, derived) {
  ...
  return joinBlocks(
    ...
    bullets([
      ...
      '**A GROUP HOLDING NOTHING BUT ICON-ONLY CONTROLS NEVER NEEDED A LINE.** The rule above answers a lone icon among labelled buttons: pair it, or label it. It has no answer when the group has no other member, and the reading that follows is to give it the line anyway. A group of icon-only controls is one or two squares wide, so it fits beside any heading at any width, and the collapse was never about it. Keep it on the heading row and let it take the cap-band alignment there. Measured on a settings header at 296px to 896px: on its own line it left 218px to 626px empty beside a 36px button, and the header stood 53px taller than it needed to be. Gate the collapse on the group holding a labelled button, so a group without one falls through to the beside-the-heading rules that were already right for it.',
...
/* ── MEASURE THE ROW PLANES, NEVER STATE THEM AS CONSTANTS ──
 *
 * These three numbers were typed into the prose and spent a session being
 * false. The rule was written when the stripe read 1.13:1 against the surface
 * and the selection 1.27, with 1.12 between them. The palette moved and the
 * document went on promising the old figures: measured today, 1.04, 1.15 and
 * 1.20. A number the document computes cannot drift from the palette it
 * describes, and the RELATIONSHIP was always the rule.
 *
 * Both modes, because a ratio measured in one says nothing about the other. */
function rowPlaneProse (state, derived) {
  const roles = derived.roles ?? {}
  const modes = [hasLight(state) ? "light" : null, hasDark(state) ? "dark" : null].filter(Boolean)
  const said = []
  for (const mode of modes) {
    const R = roles[mode] ?? {}
    if (!R.surface || !R["row-stripe"] || !R.selected) continue
    const r2 = n => n.toFixed(2)
    const stripe = r2(check(R.surface, R["row-stripe"]).ratio)
    const sel = r2(check(R.surface, R.selected).ratio)
    const between = r2(check(R["row-stripe"], R.selected).ratio)
    /* THE HOVER IS THE THIRD PLANE, derived like the other two. It is absent
       from a document that publishes no such role, so the clause is too. */
    const hov = R["row-hover"] ? r2(check(R.surface, R["row-hover"]).ratio) : null
    said.push(`in ${mode} the stripe reads ${stripe}:1 against the surface`
      + (hov ? `, the hover ${hov}` : '')
      + ` and the selection ${sel}, with ${between} between the stripe and the selection`)
  }
  return said.join("; ")
}
...
function componentsBody(state, derived) {
  ...
  const alignment = [
    ...
    bullets([
      ...
      '**A CARD WHOSE WHOLE CONTENT IS A TABLE PADS NOTHING, AND IT SAYS SO.** Take `card-bleed`: its padding is zero, the header fill and the row rules run to its edges, and the outer cells supply the gutters. That is the shape the two rules above describe. An ordinary padded card is the other case, and there the card has already supplied the space outside the table. **No selector can ask an ancestor for its padding**, so the card carries the name and any rule that depends on the difference keys off it.\n\n**THE START EDGE AND THE END EDGE ARE NOT ONE CASE.** A start-aligned first cell with no start padding puts its text ON the content edge, where the heading above it sits, and it reads as planted. An end-aligned last cell with no end padding puts its digits on the far content edge with nothing beyond, and it reads as falling off. Zeroing both "so the two are equal" was tried twice here and reported both times. The card\'s own padding cannot stand in for the end gutter, because that padding is shared with everything else in the card. So the end keeps the cell gutter and the two outer insets are deliberately unequal: measured, ink at 25 from the card border at the start and 41 at the end.\n\n**AND ON A PADDED CARD THE SELECTED ROW\'S BAR PAINTS IN THAT MARGIN**, at exactly one bar width out, so it sits flush against the row\'s own fill rather than floating off it as a detached tick. That frees the first column to keep the margin its heading sets. Measured: the bar painting 21 to 25 from the card border inside a 24px inset, the row fill starting at 25, the first column\'s ink at 25, and 28px clear between the bar and the checkbox so the two cannot fuse. On a bleed card there is no margin to use, so the column reserves the gutter instead.\n\n```css\n.card:not(.card-bleed) .table tr.is-selected > td:first-child::before {\n  inset-inline-start: calc(-1 * var(--cmp-table-row-selected-edge-width));\n}\n```',
      ...
      '**A ONE-LINE SLOT IS NOT THE CAP BAND, AND IT IS THE MECHANISM A BUILDER REACHES FOR.** Give the mark a box exactly one line tall, centre inside it, and the result looks equivalent to the formula above. It is not, for the reason the spacing section already gives about a text box\'s leading, so the slot\'s centre lands above the band\'s. Measured on five alerts built that way: 5.5px above the cap line against 0 below the baseline, so every mark sat 2.75px high. Nothing in `em` repairs it from inside the slot, because that distance needs the FONT\'S ascent and CSS cannot name it. Anchor on the baseline, which CSS does know.',
      '**AND WHERE THE ROW THEN HOLDS A BOX AS WELL AS A MARK, THE BOX DECLARES ITSELF OUT.** Putting the row on the baseline for the mark\'s sake moves anything else in it. An alert holding a trailing button measured 5.5 above and 6.25 below on its one-line slot, which is 0.375px off centre and needs no repair, and the row\'s new baseline took it to 4.63 against 7.13. `align-self: flex-start` on that button keeps it exactly where it was. Two children, two alignments, one row. Three other answers were measured and each moved it further: a cap-band transform on the button gave -1.25 against 13, and so did a baseline strut.',
      ...
      /* ── THIS SENTENCE USED TO SAY THE OPPOSITE OF THE ONE ABOVE IT ──
       *
       * It read "a tab strip scrolls, it never wraps" and told the builder to
       * give the strip `overflow-x: auto`. The sentence directly above says to
       * replace the bar rather than bolting a scrollbar onto it. A reader acts
       * on whichever they meet first, and the two shipped adjacently for as
       * long as both existed.
       *
       * The dropdown is the answer. It is what this system's own preview does
       * — nothing there carries a scroller — and it is what "sideways
       * scrolling is a last resort, never a tool" already said. So the
       * never-wraps half survives and the scroller half is gone. */
      'A **tab strip never wraps**, either. A nav is one line of destinations, and the strip\'s shape is what says so. Folded to two rows it stops reading as one control, and the marker on row two looks like a different thing: measured at 92px tall over two rows for four tabs in a 248px pane. Give the strip `flex-wrap: nowrap` and let the swap above handle the width, rather than a scroller: a destination scrolled out of view is a destination nobody visits, and a scrollbar takes 10px of height from one strip and not the one beside it, so two strips stop agreeing.',
      ...
      'A **gutter between columns is a step of its own**, never the row\'s default. Two tab strips 8px apart, with tabs 4px apart inside each, read as one long strip. Take the gutter up the scale until the ratio clears the proximity bar — 24px against 4px separated the columns, at six to one.',
      '**AND A SHARED ROW CLASS PUBLISHES THE GAP FOR A RUN OF LIKE THINGS, WHICH IS THE GAP INSIDE A GROUP.** So a row whose CHILDREN are groups needs a second class stating the between-groups step, or it hands both distances the same number. Measured on a toolbar holding a search field and an action group: 8px between them against 8px inside each, 1.0:1, so five things read as one run. 24 against 8 is exactly three to one. State it in BOTH axes where the row wraps, because there a line break IS a group boundary — the opposite of a wrapped run of buttons, where one gap serves both axes because the lines are not groups anybody reads.',
      ...
      'WHERE THE CONTAINER GAP HAS TO STAY, MAKE THE PAIR ONE CHILD OF IT. A stack that spaces several blocks cannot zero its `row-gap` for one of them, and a child cannot ask what its parent declared. So neither one can subtract the other. Wrap the heading and its description in a plain element: the pair then owns its own small step, and the stack still separates it from what follows. Measured before the wrap: 16px on five title groups, from a 12px stack gap plus a 4px margin.',
      'ANY action stands FURTHER from the prose that explains it than anything else around it, and 16px is the distance. It is not a card rule: measured on two surfaces where the text sat in a plain row, a destructive button landed 8px under its own warning and a page action 12px under the page description. Inside a card the same distance is the widest in that card. A button pressed against the sentence that explains it reads as the last line of that sentence; at 24 it reads as a separated block rather than a card with an action in it. State one distance and use it in every card.',
      ...
      `**AND A HOVER IS TRANSIENT, SO IT SITS BETWEEN THE TWO.** The row under the pointer is feedback rather than state, so it never reaches the plane a chosen row sits on. It also has to clear the STRIPE, or it says nothing on every other row of a striped table, which is half the rows a pointer lands on. \`row-hover\` is the role, and its whole job is that middle plane.\n\n**DO NOT REACH FOR THE PAGE'S RECESSED PLANE.** This system did, and it took a reader to find it: \`bg-subtle\` is the right hover step on the PAGE, where a secondary button uses it at 1.288:1 against \`bg\`, and on a CARD the same role is two steps down. Measured 1.485:1 against the surface where the selected row measured 1.153, so a band nobody asked for was 1.29 times louder than the one they chose. One role cannot serve two grounds at one bar. Every contrast number cleared, because a ratio measures text on a fill and has no opinion about which of two fills is louder.\n\n**ASSERT THE ORDER, NEVER A RATIO.** Measured across this palette and six presets in both modes: stripe 1.03 to 1.07, hover 1.08 to 1.15, selection 1.15 to 1.23, and the order holding in all fourteen. A number pinned here fails on a palette nobody thinks is broken.`,
