...
function componentsBody(state, derived) {
  ...
  const alignment = [
    ...
    bullets([
      ...
      '**AND A ZEROED CELL CANNOT TAKE THE SELECTED PADDING AS PUBLISHED.** `table-row-selected` states its inset as a SUM — the cell\'s own padding plus the edge\'s width — so it is the right answer for a cell that kept that padding. Zero the outer cell and take that sum anyway, and the marked row adds the whole thing: measured on one build, selected rows starting 16px inside the card\'s content edge against 0 for their neighbours, for a bar 4px wide. A 16px jog down the first column, from obeying both of these rules at once. Where you have changed a cell\'s padding, add `edge-width` to YOUR value instead. It is published beside the sum for exactly this, and it assumes nothing.',
      ...
      '**AND THE EDGE COSTS PADDING, WHICH IS WHY THEY ARE ONE DECISION.** A bar drawn inside the row eats the label’s own inset, so the text has to move by exactly the bar’s width or the two touch. Draw it as an inset `box-shadow`, never a border: a border widens the row by its own width, so the selected row’s label would sit at a different inset from every other label in the list. Both `nav-item-selected` and `table-row-selected` carry the edge, and each states its padding as a SUM rather than a constant, so changing the weight moves the text with it. **Each also publishes `edge-width` on its own.** The sum is the ready-made answer and it assumes the component kept its base inset; the width is the ingredient, and it is what you add when you have changed that inset yourself.',
