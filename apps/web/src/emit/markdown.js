...
function componentsBody(state, derived) {
  ...
  const alignment = [
    ...
    bullets([
      ...
      '**ONE BUTTON SIZE FIXES ALL THREE: the box, the label and the icon.** Pick the triple and every alignment question answers itself. **The icon box takes the label size minus one step on the type scale.** A 12px label takes a 10px icon, a 14px label a 12px, a 16px label a 14px. The reason is the cap band, which is what a reader compares the mark against: it is about three quarters of the type size, and the tallest glyphs in an icon set paint nearly their whole box. An icon equal to the label therefore crosses the baseline — measured at 1.3px on a download mark beside a 12px label. One step down fits it: 9px of ink in a 9px band, sitting 0.3px above the cap line and 0.3px below the baseline.',
      '**Two button sizes must never share a label size.** If they do, one label size is answering to two icon sizes and the pairing has no rule left in it. Give each step its own type: caption, body-sm, body-md.',
