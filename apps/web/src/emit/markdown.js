...
function componentsBody(state, derived) {
  ...
  const alignment = [
    ...
    bullets([
      ...
      '**AND WHICH EDGE NEEDS A GUTTER IS A QUESTION ABOUT THE CARD, NOT ABOUT THE TABLE.** The two rules above hold for a card that pads NOTHING, where the cell edge is the card edge. A card that pads its own content has already supplied both gutters, so a cell adding one doubles the inset. Measured here on three padded cards: the last column\'s ink stood 41px from the card border and the first at 33, against a card margin of 25. Five other cards on that surface put their first ink at 25, and the table was the only one that did not.\n\nSo take `card-bleed` for a card whose whole content is a table or a run of rows: its padding is zero, the header fill and the row rules reach its edges, and the cells supply the outer gutters. Take the ordinary card padding everywhere else, and zero BOTH outer cell gutters there. **No selector can ask an ancestor for its padding**, which is why the card carries the name and the cell rules key off it.\n\n**THE ACCENT BAR THEN MOVES INTO THE CARD\'S MARGIN**, at exactly one bar width out, so it sits flush against the row\'s own fill rather than floating off it as a detached tick. That frees the selection column to sit flush as well. Measured: the bar painting 21 to 25 from the card border inside a 24px inset, the row fill starting at 25, the first column\'s ink at 25, and 28px clear between the bar and the checkbox so the two cannot fuse.\n\n```css\n.card:not(.card-bleed) .table th:last-child,\n.card:not(.card-bleed) .table td:last-child { padding-inline-end: 0; }\n.card:not(.card-bleed) .table tr.is-selected > td:first-child::before {\n  inset-inline-start: calc(-1 * var(--cmp-table-row-selected-edge-width));\n}\n```',
