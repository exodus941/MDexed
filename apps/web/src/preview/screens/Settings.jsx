  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 'var(--space-xl, 32px)', alignItems: 'start' }}>
      <nav className="stack-sm">
        {sections.map((s, i) => (
          <div key={s} className={`nav-item${i === 3 ? ' is-active' : ''}`}>{s}</div>
        ))}
      </nav>
      ...
      <div className="stack">
        ...
        <div className="card" style={{ padding: 0 }}>
          {rows.map(([title, desc, on], i) => (
            <div key={title} style={{
              ...
              <span className={`badge ${on ? 'badge-success' : 'badge-neutral'}`}>{on ? 'On' : 'Off'}</span>
            </div>
          ))}
        </div>
        ...
        <div className="row" style={{ justifyContent: 'space-between' }}>
          ...
          <div className="row">
            <button className="btn btn-secondary" disabled>Discard</button>
